...
class MaxPool(AbstractLayer):
    def __init__(self, fshape, activation, strides=1, pad):
        self.fshape = fshape
    ...
    def forward(self, inputs):
        ...
        for j in xrange(s):
            for i in xrange(s):
                fmap[:, j, i, :] = np.max(inputs[:, j * self.strides:j * self.strides + self.fshape[0], i * self.strides:i * self.strides + self.fshape[1], :, np.newaxis], axis=(1, 2, 3))
        return fmap
